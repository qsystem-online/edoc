...
insert  into `config`(`fin_id`,`fst_key`,`fst_value`,`fst_notes`,`fbl_active`) values (1,'document_folder','d:\\edoc_storage\\',NULL,1),(2,'document_max_size','102400','maximal doc size (kilobyte)',1),(3,'change_branch_level','4','Min level yang berhak melakukan change branch',1);
...
insert  into `documents`(`fin_document_id`,`fst_name`,`fst_source`,`fst_created_via`,`fin_confidential_lvl`,`fst_view_scope`,`fst_print_scope`,`fbl_flow_control`,`fin_flow_control_schema`,`fst_search_marks`,`fst_memo`,`fdt_published_date`,`fbl_flow_completed`,`fin_version`,`fst_real_file_name`,`fst_active`,`fdt_insert_datetime`,`fin_insert_id`,`fdt_update_datetime`,`fin_update_id`) values (1,'test doc 1','INT','MANUAL',3,'PRV','PRV',1,NULL,'test 1','tes 1','2019-05-04',1,3,'test.pdf','R','2019-05-04 10:45:42',1,'2019-05-04 11:36:57',3),(2,'Test Document 2','INT','MANUAL',0,'PRV','PRV',1,NULL,'test 2','Revisi 1','2019-05-04',1,1,'Ebook VOL 2 (1) (2).pdf','A','2019-05-04 18:47:52',1,'2019-05-04 18:50:47',3),(3,'Testing branch doc','INT','MANUAL',0,'CST','PRV',0,NULL,'test branch','test Branch','2019-05-12',0,0,'get_file.pdf','A','2019-05-12 23:08:47',1,'0000-00-00 00:00:00',0);
...
/*!50001 CREATE TABLE  `test`(
 `fin_id` bigint(20) unsigned ,
 `fin_document_id` bigint(20) ,
 `fin_user_id` bigint(20) 
)*/;
...
/*!50001 CREATE TABLE  `test2`(
 ...
 `fin_seq_no` int(2) ,
 `fin_user_id` bigint(20) ,
 `fst_control_status` enum('NA','RA','NR','AP','RJ') ,
 `fst_memo` text ,
 `fdt_approved_datetime` datetime ,
 `fin_version` int(3) ,
 `fst_active` enum('A','S','D') ,
 `fdt_insert_datetime` datetime ,
 `fin_insert_id` int(10) ,
 `fdt_update_datetime` datetime ,
 `fin_update_id` int(10) 
)*/;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `test` AS (select max(`document_flow_control`.`fin_id`) AS `fin_id`,`document_flow_control`.`fin_document_id` AS `fin_document_id`,`document_flow_control`.`fin_user_id` AS `fin_user_id` from `document_flow_control` group by `document_flow_control`.`fin_document_id`,`document_flow_control`.`fin_user_id`) */;

/*View structure for view test2 */

/*!50001 DROP TABLE IF EXISTS `test2` */;
/*!50001 DROP VIEW IF EXISTS `test2` */;

/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `test2` AS (select `b`.`fin_id` AS `fin_id`,`b`.`fin_document_id` AS `fin_document_id`,`b`.`fin_seq_no` AS `fin_seq_no`,`b`.`fin_user_id` AS `fin_user_id`,`b`.`fst_control_status` AS `fst_control_status`,`b`.`fst_memo` AS `fst_memo`,`b`.`fdt_approved_datetime` AS `fdt_approved_datetime`,`b`.`fin_version` AS `fin_version`,`b`.`fst_active` AS `fst_active`,`b`.`fdt_insert_datetime` AS `fdt_insert_datetime`,`b`.`fin_insert_id` AS `fin_insert_id`,`b`.`fdt_update_datetime` AS `fdt_update_datetime`,`b`.`fin_update_id` AS `fin_update_id` from (`test` `a` join `document_flow_control` `b` on((`a`.`fin_id` = `b`.`fin_id`)))) */;
